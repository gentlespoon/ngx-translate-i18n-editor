...
export class SessionPanelComponent implements OnInit {
  ...
  public openProject(): void {
    var filePath = this.electronService.remote.dialog.showOpenDialog({
      ...
      filters: [
        { name: 'Gs i18n Project File', extensions: ['json'] },
      ]
    });
  ...
  public saveProject(): void {
    if (!this.sessionService.token) {
      return alert(this.translationService.instant('saveOnlyForRegisteredUser'));
    }
  ...
  public export(): void {
    if (this.i18nService.selectedLang.length) {
      ...
      if (!dirPath || !dirPath.length || !this.electronService.fs.existsSync(dirPath[0])) {
      ...
      for (var lang of this.i18nService.selectedLang) {
        ...
        this.electronService.fs.writeFile(`${dirPath[0]}/${lang}.json`, outputJSON, err => {
          if (err) alert(err);
        });
      }
      this.electronService.remote.shell.showItemInFolder(`${dirPath[0]}/${this.i18nService.selectedLang[0]}.json`);
    } else {
